...
export class AppRedux extends BaseRedux<AppState, AppEvents> {
  ...
  onIsLoggedInStatusChanged(isLoggedIn: boolean): void {
    this.dispatchApp(this.slice.actions.isLoggedInStatusChanged(isLoggedIn));
  }

  async onAppThemeChanged(isDarkMode: boolean): Promise<void> {
  ...
  async onAppInitiated(): Promise<void> {
    await this.runReduxCatching({
      action: async () => {
        ...
        this.dispatchApp(
          this.slice.actions.appInitiated({
